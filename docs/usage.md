# nf-core/pathogen: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/pathogen/usage](https://nf-co.re/pathogen/usage)
...
```console
nextflow run nf-core/pathogen --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/pathogen
```
...
First, go to the [nf-core/pathogen releases page](https://github.com/nf-core/pathogen/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
